...
> ⚠️ **0.2.0-beta · work in progress.** The status line, hooks, `setup-panes.sh`, and the orchestrator skills all work today, driven by one `panes.conf`. Expect rough edges.
...
- **`statusline-command.sh`** — the status line. Per pane it shows: working directory, git branch + worktree name, the **active GitHub issue** (orange `#N`), a cross-worktree "→" indicator when a pane's work has moved to another worktree, the model, context usage, and optional TTS/STT status. Optional: list noisy path prefixes in `~/.claude/statusline-trim-prefixes` (one per line, `~`-notation, e.g. `~/Documents/Xcode/`) and the directory segment shows just `fourlater_pane3` instead of the full path.
